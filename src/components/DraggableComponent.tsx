...
import { DraggableComponentProps } from "../types/component";
...
const DraggableComponent: React.FC<DraggableComponentProps> = ({
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (isDragging && canvasRef.current) {
        ...
        const clampedColumn = Math.max(
          0,
          Math.min(12 - component.gridSpan, newColumn)
        );
      ...
      if (isResizing) {
        ...
        if (resizeDirection.includes("bottom")) {
          newRowSpan = Math.max(
            1,
            resizeStart.originalHeight + Math.round(deltaY / cellHeight)
          );
        }
        ...
        if (resizeDirection.includes("top")) {
          ...
          const newHeight = Math.max(
            1,
            resizeStart.originalHeight - heightChange
          );
  ...
  return (
    ...
    >
      {/* Component Content */}
      <div
        className={`w-full h-full bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden ${
